import { isSupportedChainId } from '../types/index.js';
...
export class DisputeService {
    ...
    async getDisputeData(input) {
        ...
        try {
            ...
            const evidenceResults = await Promise.allSettled(evidenceSubmissions.map(submission => this.ipfsService.getEvidenceContent(submission.URI)));
            ...
            throw new Error(`Failed to retrieve dispute data for dispute ${disputeId} on ${NETWORK_NAMES[supportedChainId]}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    validateInput(disputeId, chainId) {
