...
import { EvidenceContent, ApiError } from '../types/index.js';
import { API_ENDPOINTS, REQUEST_TIMEOUTS, DEFAULT_HEADERS } from '../utils/constants.js';

export class IpfsService {
  async getEvidenceContent(ipfsUri: string): Promise<EvidenceContent> {
    if (!ipfsUri) {
  ...
  private convertIpfsUriToHttp(ipfsUri: string): string {
